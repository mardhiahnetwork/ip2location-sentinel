<?php
/**
 * Impossible Travel Detector & 2FA Login OTP Controller
 *
...
class ImpossibleTravel {
	...
	/**
	 * Calculate Haversine distance in kilometers (alias).
	 *
	 * @param float $lat1
	 * @param float $lon1
	 * @param float $lat2
	...
	public static function is_mobile_or_carrier( array $geo_data, string $ua = '' ): bool {
		...
		if ( in_array( $ua_info['device'], array( 'Mobile', 'Tablet' ), true ) ) {
			return true;
		}

		$as_name = strtolower( $geo_data['as'] ?? '' );
	...
	public static function check_impossible_travel( $user, string $username, string $password ) {
		...
		$same_country = ( ! empty( $prev_cc ) && ! empty( $curr_geo['country_code'] ) && strtoupper( $prev_cc ) === strtoupper( $curr_geo['country_code'] ) );

		$domestic_mode = $settings['impossible_domestic_mode'] ?? 'mobile_tolerance';

		// 1. Ignore Same-Country mode: Skip velocity checks within the same country
		if ( $same_country && $domestic_mode === 'ignore_domestic' ) {
			return $user;
		}

		// 2. Smart Mobile Tolerance mode: Allow domestic hops when on dynamic mobile/telco network
		if ( $same_country && $domestic_mode === 'mobile_tolerance' ) {
			...
			// If either connection is a mobile carrier / mobile device, OR same device user-agent is logging in within same country
		...
		if ( $distance_km > $min_distance && $speed_kmh > $threshold_speed ) {
			...
			Logger::log_event(
				...
				sprintf( 'Impossible speed: %s km/h over %s km in %s hrs', round( $speed_kmh, 1 ), round( $distance_km, 1 ), round( $time_diff_hours, 2 ) ),
				$user->user_login,
				$user->ID,
				$curr_geo
			);
	...
	public static function initiate_otp_flow( WP_User $user, array $geo, array $details ): void {
		...
		$subject   = sprintf(
			...
			__( '[%1$s] Verification Code: %2$s (Unusual Login Detected)', 'ip2location-sentinel' ),
			$site_name,
			$otp_code
		);
	...
	public static function render_otp_screen(): void {
			...
			if ( wp_check_password( $input_otp, $data['secret'] ) ) {
				...
				if ( $user ) {
					...
					Logger::log_event(
						...
						'Login (wp-login.php)',
						'ALLOWED',
						'2FA OTP Challenge Verified Successfully',
						$user->user_login,
						$user->ID,
						$data['geo']
					);
		...
		<form name="ip2loc_otp_form" id="ip2loc_otp_form" action="<?php echo esc_url( add_query_arg( array( 'action' => 'ip2loc_otp', 'token' => rawurlencode( $token ) ), wp_login_url() ) ); ?>" method="post">
			...
			<p style="margin-bottom: 16px; font-size: 13px; line-height: 1.5; color: #444;">
				<strong><?php esc_html_e( 'Unusual Login Detected', 'ip2location-sentinel' ); ?></strong><br>
				<?php esc_html_e( 'An email with a 6-digit verification code has been dispatched to your registered address. Enter it below to complete your login.', 'ip2location-sentinel' ); ?>
			</p>
			<p>
	...
	/**
	 * Record user's login location upon successful authentication.
	 *
