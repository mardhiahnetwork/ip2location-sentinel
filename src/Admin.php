...
class Admin {
	...
	public static function register_admin_menus(): void {
		add_menu_page(
			__( 'LocaSentinel', 'ip2location-sentinel' ),
			__( 'LocaSentinel', 'ip2location-sentinel' ),
			'manage_options',
		...
		add_submenu_page(
			'ip2location-sentinel',
			__( 'Dashboard - LocaSentinel', 'ip2location-sentinel' ),
		...
		add_submenu_page(
			'ip2location-sentinel',
			__( 'Firewall & Rules - LocaSentinel', 'ip2location-sentinel' ),
		...
		add_submenu_page(
			'ip2location-sentinel',
			__( 'Security Audit Logs - LocaSentinel', 'ip2location-sentinel' ),
		...
		add_submenu_page(
			'ip2location-sentinel',
			__( 'Settings & Diagnostics - LocaSentinel', 'ip2location-sentinel' ),
	...
	public static function handle_save_settings(): void {
		...
		if ( $tab === 'impossible_travel' || isset( $input['impossible_speed_threshold'] ) || isset( $input['impossible_action'] ) ) {
			...
			$impossible_action                     = $input['impossible_action'] ?? '';
			$current['impossible_action']          = in_array( $impossible_action, array( 'webhook', 'webhook_only' ), true ) ? 'webhook_only' : 'otp';
