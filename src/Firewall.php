...
class Firewall {
	...
	public static function init(): void {
		...
		add_filter( 'authenticate', array( __CLASS__, 'verify_login_captcha' ), 20, 3 );
	...
	/**
	 * Verify CAPTCHA challenge upon login submission on untrusted devices.
	 *
	 ...
	 */
	public static function verify_login_captcha( $user, string $username, string $password ) {
		...
		if ( $user_obj && ImpossibleTravel::has_valid_trusted_device( $user_obj->ID ) ) {
			return $user;
		}

		if ( ! Captcha::verify_submission() ) {
			return new \WP_Error(
				'ip2loc_captcha_failed',
				'<strong>' . esc_html__( 'Error:', 'ip2location-sentinel' ) . '</strong> ' . esc_html__( 'CAPTCHA verification failed. Please complete the security check.', 'ip2location-sentinel' )
			);
		}
